...
**Hybrid preference data on helpfulness.** Several frontier teams now train reward models on a mix of human and AI preferences, using AI labels for volume and reserving human annotation for calibration and edge cases. The specifics vary by team and are not always public, but the pattern is widespread.
...
**Verifiable rewards where they apply.** DeepSeek-R1 is the counter-example that proves the rule. On maths and code, verifiable correctness plus a format hard floor outperformed judge-heavy alternatives.
...
**The circularity problem.** When the model generating the data also judges it, there is no outside check. Any systematic bias is reinforced rather than corrected. The "scalable supervision" framing assumes the judge is more capable or trustworthy than the policy on the axis being judged. When that fails, you are scaling existing biases at the speed of inference. This motivates the scalable oversight and deliberative alignment research agendas, and it is the unsolved part of the field.
