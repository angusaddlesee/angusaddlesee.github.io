...
where $C_{\text{inf}}$ is inference compute per problem. Snell et al.'s result is that for hard problems the slope $\beta$ is steep enough that the trade against $C_{\text{train}}$ is favourable across a wide range of difficulties. For easy problems it inverts — a small amount of training compute outclasses any inference compute, because the answer is one forward pass away.
...
Test-time compute is the fourth axis of LLM scaling, alongside parameters, training data, and post-training. The first three are levers you pull at training time and ship. The fourth you pull continuously, in production, on every request. That makes it the axis with the most leverage for product teams — good engineering compounds over the entire serving lifetime of the model.

It also ties the rest of this series together. Reasoning models are valuable because they use test-time compute well. Reward models are the verifiers that make search work. The routing layer is how you allocate inference compute per query. The forthcoming [agentic RL deep-dive](/blog/) is what happens when test-time compute extends to multi-step rollouts with tools. Facets of the same problem.

The short version: training-compute scaling did not stop working — it stopped paying back fast enough to be the only thing worth doing. Test-time compute is the lever that opened up next. The compute has moved. The question is whether your serving stack has noticed.
