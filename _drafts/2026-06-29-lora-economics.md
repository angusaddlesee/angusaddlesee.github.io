...
In 2020 the cost of fine-tuning a 70B model was a research grant. By 2024 it was a long weekend on a single rented A100. What closed the gap was not Moore's law, a new accelerator, or a better optimiser — it was a one-page idea about rank. Hu et al. wrote LoRA, Dettmers et al. bolted 4-bit quantisation onto it eighteen months later, and the economics of LLM customisation re-folded around two small matrices and a quantile-spaced number format. Frontier labs still do full fine-tuning when it matters. Almost nobody else does, and most of them never need to.
...
2. **Double quantisation** — quantising the per-block quantisation _constants_ themselves at 8-bit. Small in absolute terms (~0.13 bits per parameter) but it adds up.
...
The forward pass becomes $h = \text{dequant}(W_{\text{4bit}}) x + (\alpha/r) B A x$, with $W_{\text{4bit}}$ frozen and the LoRA matrices in bf16. The 65B model Dettmers fine-tuned on a single 48GB GPU would have needed ~780GB in 16-bit full fine-tuning. That is a 16× memory reduction at negligible quality cost, and the ratio is the point: QLoRA didn't make fine-tuning a bit cheaper, it changed which hardware tier the problem lived on. The discontinuity wasn't that 7B fine-tuning got faster — it was that 65B fine-tuning became something a graduate student could do.
...
**LoRA+ (Hayou et al., 2024)** is the closest thing PEFT has to a free lunch. Because $A$ is Gaussian-initialised and $B$ is zero, the gradients to $A$ are proportional to $\|B\|$ (zero at start) and the gradients to $B$ are proportional to $\|A\|$ (non-zero). The matrices have asymmetric learning dynamics; giving them the same learning rate is provably suboptimal. LoRA+ sets $\eta_A = \lambda \cdot \eta_B$ with $\lambda \approx \sqrt{d/r}$ — practically, $\lambda \in \{4, 8, 16\}$ depending on model size. One optimiser parameter group, 1–2% accuracy, 10–20% faster convergence. There is no reason not to use it; the only reason most people don't is that they haven't heard of it.
...
**Tasks where the last 1–2% is load-bearing.** LoRA reaches 90–95% of full fine-tuning quality on most benchmarks, and the gap is usually fine. But for the frontier-grade post-training that defines a flagship model, the last percentage points are the entire point of the exercise. Frontier labs do full fine-tuning for headline alignment runs and LoRA for the 90% of experiments that aren't. That is the right division.
...
LoRA is the substrate underneath most non-frontier LLM customisation, and QLoRA is what made that substrate accessible at scale. The combination is one of the few cases in modern ML where the practical impact has comfortably outrun the theoretical — the rank-decomposition idea is mathematically straightforward, the implementation is short, and the economic consequences have been enormous. It changed which experiments are run, by whom, and for which reasons.

What it has not done is dissolve the quality gap with full fine-tuning at the absolute frontier. The labs running headline alignment runs still pay the full bill. What LoRA did is make that gap matter to a much smaller fraction of the field. For the rest of us — applied teams customising open models for verticals, academics running ablations, indie developers shipping personalised assistants — fine-tuning is now closer to "configure and run" than to "raise capital and plan."
