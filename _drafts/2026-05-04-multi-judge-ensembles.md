...
**Majority vote.** For pairwise with $k$ judges, the response preferred by at least $\lceil k/2 \rceil$ wins. For pointwise, take the median — robust to outliers in a way the mean is not. Right starting point.
...
The cheapest way to measure correlation is on the calibration set you used for the bias audit in the [position-bias post](/blog/2026/pairwise-position-bias/). Run every judge on every example. Compute Cohen's kappa between every pair. Above 0.8: essentially the same judge — drop one. 0.4–0.7: healthy disagreement. Below 0.3: at least one is not actually competent.
...
Multi-judge ensembles are the natural successor to the position-debiasing work in the [pairwise post](/blog/2026/pairwise-position-bias/). They turn a calibrated single judge into an evaluation pipeline you can trust to drive deployments, and they are the prerequisite for a [judge-distilled reward model](/blog/2026/reward-modelling-at-scale/) that does not silently inherit one judge's blind spots. They feed directly into the [calibration and drift monitoring](/blog/) discipline that keeps the stack honest as the underlying models update underneath you.
