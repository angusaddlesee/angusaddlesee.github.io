...
Two properties to carry. First, the curve is _predictable_: the qualitative pattern of initial improvement followed by degradation holds across reward model sizes, tasks, and training algorithms, even though the exact functional form differs between best-of-$n$ and RL. The coefficients scale smoothly with reward-model size, which is enough to anticipate roughly where the cliff sits (though Gao et al. caution that their RL form misbehaves near the origin and that extrapolation to higher KL underestimates). Second, larger reward models earn a higher peak gold score, so scale buys quality headroom, but note that Gao et al. find the peak arrives at an _earlier_ KL distance as the reward model grows, not a later one.
