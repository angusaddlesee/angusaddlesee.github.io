...
tags: rlhf grpo reinforcement-learning reward-modeling alexa
...
**GSPO** is the variant for teams with DPO infrastructure. Instead of a policy-gradient update, it constructs preference pairs from the group — best-reward as $y_w$, worst-reward as $y_l$ — and applies a DPO-style loss:

$$\mathcal{L}_{\text{GSPO}} = -\log \sigma\!\left(\beta \!\left[\log \frac{\pi_\theta(y_w \mid x)}{\pi_{\text{ref}}(y_w \mid x)} - \log \frac{\pi_\theta(y_l \mid x)}{\pi_{\text{ref}}(y_l \mid x)}\right]\right).$$

You lose the fine-grained signal from the middle of the group, but you gain DPO's well-understood optimisation profile and dodge the clipping/advantage-normalisation hyperparameter dance. For teams whose DPO infrastructure is more mature than their PPO infrastructure, GSPO is often the easier path to on-policy training.

The pattern is the same across the family: sample a group, score the group, do _something_ with the group statistics. Pick whichever loss matches the optimisation tooling you already trust; the group-relative trick is doing the heavy lifting either way.
...
8. **For preference-pair tasks, evaluate GSPO before defaulting to GRPO.** If your infrastructure is DPO-heavy, GSPO gives you on-policy generation with the loss machinery you already trust.
