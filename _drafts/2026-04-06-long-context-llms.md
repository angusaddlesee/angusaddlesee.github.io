...
Second, an attention-temperature correction: longer sequences diffuse the attention distribution and make the model less decisive, so YaRN scales the attention logits by $\sqrt{1/t}$, with the temperature $t$ set by $\sqrt{1/t} = 0.1 \ln(s) + 1$. For a 4× extension that works out to a factor of roughly 1.14, a modest sharpening that pushes back against the entropy increase from longer sequences. Both adjustments are tiny in implementation and meaningful in results: the paper reports strong passkey retrieval and perplexity at extended lengths with only ~400 fine-tuning steps, where Position Interpolation needs substantially more training to get comparable results. If you're extending an open-weight model in 2026, YaRN is the default.
...
None of these change the physics. Quality degrades smoothly with extension ratio: in my experience, modest extensions (2–4×) are nearly free, and the further past that you push, the steeper the curve gets. By 32×, where 4K becomes 128K, lost-in-the-middle stops being a curiosity and becomes the dominant failure mode.
