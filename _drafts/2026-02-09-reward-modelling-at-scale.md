...
**Process reward models (PRMs).** Step-level scoring rather than outcome-level. The reward model produces a score at each step boundary in a multi-step response, and the response-level score is the product of the per-step correctness probabilities (the probability that every step is correct; taking the minimum, the weakest-link reading, performs comparably). The classic result is Lightman et al. (2023)'s "Let's Verify Step by Step": on the MATH benchmark, a PRM-guided best-of-N selection solved 78.2% of problems versus 72.4% for an ORM under the same best-of-N search budget, though the two reward models' training sets are, as the authors note, not directly comparable. The cost is annotation: step-level labels are 5–10× more expensive than preference pairs because each step has to be verified independently. PRMs are the right tool for verifiable reasoning domains (maths, code, formal logic). For open-ended dialogue or creative writing, the lack of natural step boundaries makes them awkward to apply.
...
**Multi-headed and multi-axis reward models.** Several scalar reward signals (one for helpfulness, one for harmlessness, and so on) combined at the policy-optimisation step with tunable weights. Llama 2 runs a version of this pattern, training separate helpfulness and safety reward models and combining them. The advantage is that you can re-weight the trade-off between axes without retraining. The disadvantage is that each axis still inherits the Bradley-Terry single-dimension assumption per head, and you have to be honest about whether your "axes" are really independent or just slices of the same latent quality scale.
