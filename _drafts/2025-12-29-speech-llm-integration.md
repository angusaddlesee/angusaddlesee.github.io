...
The cascade is not just a bag of models; it is a bag of _capabilities_ that production teams accumulated over years. Every one of them has to be preserved or replaced during migration, and "we'll figure it out later" is the answer that ends migrations.
...
Future posts in this series will go deeper into each piece: audio tokenisation choices, the speech-text interleaving design space, streaming architectures, full-duplex training curricula, and the operational mechanics of a production migration. If you're weighing up when to start, my honest advice is that the diagnostic work of Phase 1 is cheap enough, and the lead time long enough, that the right moment to begin measuring is probably now.
