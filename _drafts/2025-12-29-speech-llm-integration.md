...
[Contextual biasing](/blog/) is the next biggest. Production ASR systems have years of work in shallow-fusion biasing toward user-specific entities — your contacts, your playlists, the unusual restaurant you've been talking about. Collapse the ASR and that machinery is gone. The replacements are entity lists in the LLM context, attention-based biasing on the speech encoder, or retrieval-augmented recognition. None are drop-in replacements for a tuned ASR biasing stack. This is the area where I expect production migrations to spend the most engineering time per unit reward.
...
For technical leaders, the strategic point: the cascade is on borrowed time, but borrowing strategically is not the same as borrowing badly. The teams that migrate well will be the ones that treat this as a multi-year program with discipline at every phase, not a chase for the frontier headline. The teams that migrate badly will be visible. There is no quiet way to break a voice assistant.
