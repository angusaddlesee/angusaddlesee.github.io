...
**Confidence-based gates.** Use the small model's log-probabilities as a proxy for "did this go well?": mean token log-probability, the probability of the lowest-probability token, or perplexity. The appeal is that it's free. The catch is calibration: models can be eloquently incorrect, especially on factual questions where the next-token distribution is tight around a wrong answer. Confidence is a useful signal, not a true one.
